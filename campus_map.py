        ...
        for j in range(len(array)-1):
            ...
            array[i].location.strip() == array[j].right.strip():
                if array[j].location.strip() not in path:
                    #Look for other arrays with same destination
                    k = 0
                    for k in range(len(array)-1):
                        if array[i].location.strip() == array[k].left.strip() or \
                        array[i].location.strip() == array[k].right.strip():
                            if k != i:
                                #Choose the one with the best time
                                if array[k].time > array[j].time:
                                    path.insert(0, array[j].location.strip() )
                                    time += int(array[i].time)
                                    i = j
                                else:
                                    path.insert(0, array[k].location.strip() )
                                    time += int(array[i].time)
                                    i = j
                        else:
                            k=k+1
...
def solution(path, time):
    """
    Generates the final path with the solution for the quickest route
    ...
    m,s = divmod(time, 60)
    st.write('This path will take ', m, ' minutes and ', s, ' seconds to reach your destination')
